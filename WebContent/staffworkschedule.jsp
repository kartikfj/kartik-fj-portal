...
<c:choose>
    <c:when test="${!empty fjtuser.emp_code  and fjtuser.checkValidSession eq 1 and ( fjtuser.role eq 'hr' or fjtuser.role eq 'hrmgr' )}">  
        ...
        <c:if test="${pageContext.request.method eq 'POST'}">                     
            <c:choose>
                <c:when test="${param.operationbutton eq 'savesch'}">
                    ...
                    <jsp:setProperty name="newsch" property="type" value="STAFF"/>
        ...
        <sql:query var="rs" dataSource="jdbc/mysqlfjtcolocal">
            SELECT valid_from ,valid_to, min(param_value) as starttime, max(param_value) as endtime FROM  system_params where (param_name= 'STAFF_DAY_START' or param_name= 'STAFF_DAY_END') and status=1 and company_code=? group by valid_from,valid_to
        ...
        <html>
            <head>
                ...
                <script>
                    $(function () {
                        $("#datepicker-2").datepicker({minDate: new Date('${newsch.lastScheduleStartDateForStaff}'), maxDate: '+1Y', dateFormat: 'dd/mm/yy'});
            ...
            <body>
                <div class="container">
                    <div class="col-xs-12 " style="padding-bottom: 10px;padding-top:-25px;">  
			       		<a href="staffworkschedule.jsp" class="button">Staff Work Schedule</a>
		                <a href="labourworkschedule.jsp" class="button">Labour Work Schedule</a>
		                <a href="holidaycalendar.jsp" class="button">Holiday calendar</a>
		                <a href="editsysparams.jsp" class="button">Processing month settings</a>
		                <a href="muster.jsp" class="button">Muster roll</a>
		                <a href="LeaveCancellation" class="button">Leave Cancellation</a>
		                <a href="HrRegularisationCorrectionController" class="button">Regularisation Correction</a>
		                <a href="backdatedLeaveappln.jsp" class="button">Back Dated Leave Apply</a>	            
			       	</div>	
			       	       	
                    <h1>Work schedules : Staff &nbsp;
                    ...
                    <c:if test="${pageContext.request.method eq 'POST' and param.operationbutton eq 'savesch'}">   
                        <c:choose>
                            ...
                            <c:otherwise>
                                <div class="n_text_msg" style="float:left;" id="messagebox">
                        ...
                        <form class="form-horizontal" method="post" onsubmit="return verifySchedule();">   
                            <div class="form-group">
                                ...
                                <div class="col-sm-3"><input type="text" class="form-control" id="datepicker-3" name="schto" value="31/12/2099" /> </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="col-sm-3"> <input class="form-control" id="staff_fromtime" name="staff_fromtime" size="7" /> </div>
                               <label  class="col-sm-1">to</label>
                                   <div class="col-sm-3"><input class="form-control" name="staff_totime" id ="staff_totime" size="7" />
                                   </div>
                            </div>
                            <div class="form-group">
