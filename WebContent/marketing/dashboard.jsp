...
<head>
	...
	<script type="text/javascript" src="././resources/js/mkt-dashboard.js?v=18062020"></script>
  ...
  <link rel="stylesheet" href="././resources/dist/css/skins/_all-skins.min.css">
   <link rel="stylesheet" href="././resources/css/mkt-dashbaord.css?v=18062020">
 <script>
 var defStartDate = '';var defEndtDate = '';
 $(function(){ defStartDate = '${defaultStartDate}' ; defEndtDate = '${defaultEndDate}' ; var title ='FJ-Marketing dashboard sales lead  report - from';
 $('#mkt-dashboard-table').DataTable( { dom: 'Bfrtip', "paging":false,  "ordering": false,"info":false,"searching": false,buttons: [{
 extend: 'excelHtml5',  text:      '<i class="fa fa-file-excel-o" style="color: #065685; font-size: 1.5em;">Export</i>', filename: title+' '+defStartDate+' to '+defEndtDate+'',
 title: title+' '+defStartDate+' to '+defEndtDate+'', messageTop: 'The information in this file is copyright to Faisal Jassim Group.'}]});});
  </script>
 <style>
    ...
    <section class="content">
      ...
      <c:if test="${totalUnqLdCount ne null or !empty totalUnqLdCount }">
         ...
         --%>
           <c:if test="${SFO eq 1}">
          <form method="POST">
          <c:if test="${fjtuser.role eq 'mkt' or fjtuser.role eq 'mg' }">
          <div class="col-md-2 col-xs-6">          
	...
	<%-- Tab area start --%>	
	 <div class="nav-tabs-custom">
            ...
            <ul class="nav nav-tabs pull-right">
              <li class="report-title"><a href="#yearly-details" data-toggle="tab">Sales Lead Report from  ${defaultStartDate}  to ${defaultEndDate}</a></li>
              <%-- <li class="pull-center header report-title"><i class="fa fa-inbox"></i> </li>--%> 
            </ul>
          <%-- Yearly tab start --%>
          <div class="tab-content no-padding">
              <div class="tab-pane active" id="yearly-details" style="position: relative;">
      		   ...
      		   <c:set var="rowclassname" value="mkd-final-row" scope="page" />
      			...
      			<td style="width:max-content !important;">        					
		      			...
		      			<c:if test="${totalCount ne rowCountNew}">
		      				<a href="#" class="toggler view-more" data-lead-cat="${data.division}"  ><i class="fa fa-plus" ></i></a>
		      			</c:if>      		 			
      			</td>
      			<td>${data.leadType}Stage2 & New Opp.</td>
      			<td class="number-apperarnce">${data.totalLeads}</td>
      			<td class="number-apperarnce">${data.ackPendingLeads}</td>
      			<td class="number-apperarnce">${data.qtdLeads}</td>
      			<td class="number-apperarnce">${data.declinedLeads}</td>
      			<td class="number-apperarnce">${data.lostLeadsAtOV}</td>
      			<td class="number-apperarnce">${data.stage3Count}</td>
      			<td class="number-apperarnce"><fmt:formatNumber pattern="#,###.##" value="${data.stage3Value}" /></td>
      			<td class="number-apperarnce">${data.lostLeadsAtS3}</td>
      			<td class="number-apperarnce">${data.stage4Count}</td>
      			<td class="number-apperarnce"><fmt:formatNumber pattern="#,###.##" value="${data.stage4Value}" /></td>
      			<td class="number-apperarnce">${data.stage4NotConfrmd}</td>
      			<td class="number-apperarnce">
      			     <c:choose>
      		  ...
      		  <c:if test="${totalCount eq rowCountNew and DBT eq 2 }">
      		   <c:set var="rowclassname" value="mkd-final-row" scope="page" />
      			...
      			<td style="width:max-content !important;">     		     			      				
		      			...
		      			<c:if test="${totalCount ne rowCountNew}">
		      			${data.seEmpName} 
		      				<a href="#" class="toggler view-more" data-lead-cat="${data.seEmpCode}"  ><i class="fa fa-plus" ></i></a>
		      			</c:if>	      			  			
      			</td>
