...
function evalFieldsForCalculatebtn(form1){
 var oprn = document.getElementById("calbutton");
  ...
  // alert("total lv "+totallv+"nolvd "+nolvd+" total days "+totalDays);
   ...
   if( tot < noofdays && leaveselect !='AN30' && leaveselect !='AN60' || tot < noofdays ||  tot <= 0){
...
function manageFormElements(type){
   ...
   }else if(leaveselect.value == 'CASUAL' || leaveselect.value == 'CHRISTMAS' || leaveselect.value == 'AN30' || leaveselect.value == 'AN60'){ //-- nufail start
...
function isHalfDayApplicable(){
    ...
    if(diff == 0){
        ...
        if(currentleavesel== 'CASUAL' || currentleavesel== 'SLV'){       
           return true;
        }else {    
...
function updateLeaveBalance(){
    ...
    msgdiv.innerHTML = "Current "+leaveselect.options[leaveselect.selectedIndex].text+ " balance : "+balance;
