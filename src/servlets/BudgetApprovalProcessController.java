...
/**
 * Servlet implementation class BudgetApprovalProcessController
 */
@WebServlet("/SOProcess")
public class BudgetApprovalProcessController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	BudgetApprovalMailDbutil budgetApprovalMailDbutil;

	/**
	 * @throws ServletException
	 * @see HttpServlet#HttpServlet()
	 */
	public BudgetApprovalProcessController() throws ServletException {
		super();
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if (!approver.equals("null") && !so_created_dt.equals("null") && !operation_type.equals("null")) {
			...
			requsetSOStatus = budgetApprovalMailDbutil.soStatus(so_id);
			...
			if (requsetSOStatus >= 1) {
				...
				case "1":
					...
					result = budgetApprovalMailDbutil.updateApproveRejectStatus(so_id, approver, updateApproved);
					// System.out.println("UPDATE RESULT "+result);
					if (result == 1) {
						int aaproveResult = budgetApprovalMailDbutil.sendApprovalOrRejectStatusReplyMail(so_id,
								updateApproved);
						System.out.println("Reply Mail Approve SEND Status : ? = " + aaproveResult);
						request.setAttribute("MSG",
								"<b style='color:green;'> SO Budget is Approved successfully. Thank You .</b>"
				...
				case "2":
					...
					result = budgetApprovalMailDbutil.updateApproveRejectStatus(so_id, approver, updateRejected);
					// System.out.println("UPDATE RESULT "+result);
					if (result == 1) {
						int rejectResult = budgetApprovalMailDbutil.sendApprovalOrRejectStatusReplyMail(so_id,
								updateRejected);
						System.out.println("Reply MAIL Rejected SEND? " + rejectResult);
						request.setAttribute("MSG",
								"<b style='color:green;'> SO Budget is Rejected Successfully. Thank You .</b>"
										+ "<footer>Copyright © FJ-Group</footer>");
					}
